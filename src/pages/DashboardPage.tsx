    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <header className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">Here's an overview of your academic deadlines and tasks.</p>
        </div>
        <Link 
          to="/add-task" 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <StatsCard 
          ...
          icon={AlertCircle} 
          colorClass={collisions.length > 0 ? "bg-red-100 text-red-600" : "bg-slate-100 text-slate-600"}
          delay={0.4}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="space-y-4">
            {collisions.length === 0 ? (
              <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-2xl text-center">
                ...
                <p className="text-emerald-800 font-medium">No collisions detected! You're good to go.</p>
              </div>
            ) : (
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/tasks" className="text-primary-600 font-semibold hover:underline text-sm">View All</Link>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            {tasks.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-slate-500">No tasks yet. Start by adding one!</p>
              </div>
            ) : (
                ...
                {tasks.slice(0, 5).map((task) => (
                  <div key={task.id} className="p-4 hover:bg-slate-50 transition-colors flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-bold text-slate-900 group-hover:text-primary-600 transition-colors">{task.title}</h4>
                        <div className="flex gap-3 mt-1">
                          <span className="text-xs text-slate-500 font-medium">{task.subject}</span>
